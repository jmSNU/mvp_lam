...
@dataclass
class PaddedCollatorForActionPrediction_Simpler:
    model_max_length: int
    pad_token_id: int
    padding_side: str = "right"
    pixel_values_dtype: torch.dtype = torch.float32
...
@dataclass
class PaddedCollatorForActionPredictionScratch_Simpler:
    model_max_length: int
    pad_token_id: int
    padding_side: str = "right"
    pixel_values_dtype: torch.dtype = torch.float32
    ...
    def __call__(self, instances: Sequence[Dict[str, Dict[str, torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        if "state" in instances[0]:
            ...
            for inst in instances:
                s = torch.from_numpy(inst["state"])  # e.g. [T, S]
                state_list.append(s)

                ds_name = inst.get("dataset_name", None)
                if ds_name is not None:
                    ds_key = ds_name.decode("utf-8") if isinstance(ds_name, bytes) else str(ds_name)
                    if ds_key not in state_by_dataset:
                        state_by_dataset[ds_key] = []
                    state_by_dataset[ds_key].append(s)
            state_by_dataset = {k: torch.stack(v, dim = 0) for k, v in state_by_dataset.items()}
            output["state_by_dataset"] = state_by_dataset
