        ...
        if not self.use_viewaug:
            ...
            for k, v in obs.items():
                if not k.startswith("image"):
                    continue
                if isinstance(v, np.ndarray) and v.ndim >= 4 and v.shape[0] >= 2:
                    image_keys.append(k)
                elif isinstance(v, (list, tuple)) and len(v) >= 2:
                    image_keys.append(k)

            if len(image_keys) == 0:
                raise ValueError("No multi-view image keys found in rlds_batch['observation'].")

            view_key = image_keys[int(torch.randint(0, len(image_keys), (1,)).item())]
            frames = obs[view_key]

            img = Image.fromarray(frames[0])
            img_k = Image.fromarray(frames[-1])
            ...
            action_vocab_len = latent_action_idx.numel()
            ...
            labels[: -(action_vocab_len + 1)] = IGNORE_INDEX
            if not self.predict_stop_token:
                labels[-1] = IGNORE_INDEX
...
@dataclass
class RLDSBatchTransformSimpler:
    action_tokenizer: ActionTokenizer
    base_tokenizer: PreTrainedTokenizerBase
    image_transform: ImageTransform
    image_transform_lam: ImageTransform
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with torch.no_grad():
            ...
            target_pixel_values= self.image_transform_lam(img_k)
            video = torch.stack([initial_pixel_values, target_pixel_values], dim=0).unsqueeze(0).to(self.action_tokenizer.device)
            latent_action_idx = self.action_tokenizer.vq_encode(video)['indices'].squeeze() # [4]
...
class RLDSBatchTransformMultiViewVideo:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "proprio" in rlds_batch["observation"].keys():
            state = np.array(rlds_batch["observation"]["proprio"])
        else:
            state = None
        lang = rlds_batch["task"]["language_instruction"].decode().lower()
        ...
        for k in rlds_batch['observation'].keys():
            if k.startswith("image_"): # image_primary, image_secondary, image_wrist
                ...
                # the frame interval is already tackled in RLDS dataloader
                target_frame_index = -1
                img_k = Image.fromarray(rlds_batch["observation"][k][target_frame_index])#.copy()
                # print(sum(np.array(img_k) - np.array(img)))
                target_pixel_values= self.image_transform(img_k)
                ...
                out[view_name] = dict(initial_pixel_values=initial_pixel_values, target_pixel_values=target_pixel_values, 
                    task_instruction=lang, action=action)
        out["coeff"] = coeff
