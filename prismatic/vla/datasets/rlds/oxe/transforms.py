...
def perturbed_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    for key in trajectory.keys():
        if key == "traj_metadata":
            continue
        elif key == "observation":
...
def simpler_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """
    Your SimplerRLDS TFDS schema (per-step):
      observation: { image, language_instruction }
      action: (7,)
    After RLDS batching:
      trajectory["observation"]["image"]                : (T, H, W, 3)
      trajectory["observation"]["language_instruction"] : (T,)
      trajectory["action"]                              : (T, 7)
    """
    if "language_instruction" in trajectory["observation"]:
        trajectory["language_instruction"] = trajectory["observation"].pop("language_instruction")

    a = tf.cast(trajectory["action"], tf.float32)  # (T, 7)
    a = a[:, :7]

    g = tf.clip_by_value(a[:, -1], 0.0, 1.0)
    g = binarize_gripper_actions(g)  # (T,)
    a = tf.concat([a[:, :6], g[:, None]], axis=-1)
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "bridge_rlds": bridge_orig_dataset_transform,
    "bridge_multiview": bridge_orig_dataset_transform,
    "bridge_50": bridge_orig_dataset_transform,
    "bridge_10": bridge_orig_dataset_transform,
    "rlds_nvs_perturbed": perturbed_dataset_transform,
    "libero_suite_perturbed": perturbed_dataset_transform,
...
egoexo4d_transform = {f"egoexo4d_split_{i}": human_dataset_transform for i in range(1, 53)}

OXE_STANDARDIZATION_TRANSFORMS.update(egoexo4d_transform)