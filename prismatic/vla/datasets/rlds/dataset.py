...
def count_monochrome_images_in_traj(
    traj: dict, 
    eps: float = 1e-3
) -> tf.Tensor:
    obs = traj["observation"]
    mono_count = tf.constant(0, tf.int32)
    ...
    for k, v in obs.items():
        ...
        if v.dtype == tf.string:
            continue
        img = tf.cast(v, tf.float32)
        rank = tf.rank(img)
        spatial_axes = tf.range(1, rank)

        frame_min = tf.reduce_min(img, axis=spatial_axes)
        frame_max = tf.reduce_max(img, axis=spatial_axes)

        diff = frame_max - frame_min
        is_mono = tf.less_equal(diff, eps)          
        mono_count += tf.reduce_sum(
            tf.cast(is_mono, tf.int32)
        )
    return mono_count
            ...
            new_obs["proprio"] = tf.concat(
                [
                    (
                        ...
                        if key is None or key not in old_obs
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    def _keep_traj(traj: dict) -> tf.Tensor:
        mono_count = count_monochrome_images_in_traj(
            traj, eps=1e-3
        )
        return mono_count < 10
    dataset = dataset.filter(_keep_traj)
