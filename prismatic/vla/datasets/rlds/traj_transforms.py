...
def chunk_act_obs_full_action(
    ...
    """
    For each timestep t:
      - observation -> indices [t, t + window_size - 1]
      - action      -> indices [t, t+1, ..., t + window_size + future_action_window_size - 1]

    Indices are clipped to [0, traj_len - 1] and for actions also to goal_timestep.
    Actions past goal_timestep are neutralized as in the original code.
    """
    traj_len = tf.shape(traj["action"])[0]
    action_dim = traj["action"].shape[-1]

    first_indices = tf.range(traj_len)[:, None]  # shape: [T, 1]
    last_indices = first_indices + (window_size - 1)  # [T, 1]
    chunk_indices = tf.concat([first_indices, last_indices], axis=1)  # [T, 2]

    floored_chunk_indices = tf.maximum(
        tf.minimum(chunk_indices, traj_len - 1),
        0,
    )

    total_action_window = window_size + future_action_window_size
    # [T, 1] + [1, total_action_window] -> [T, total_action_window]
    action_chunk_indices = (
        tf.range(traj_len)[:, None]
        + tf.range(total_action_window)[None, :]
    )

    if "timestep" in traj["task"]:
        goal_timestep = traj["task"]["timestep"]  # shape: [T]
    else:
        goal_timestep = tf.fill([traj_len], traj_len - 1)
    ...
    absolute_action_mask = traj.get(
        ...
        tf.zeros([traj_len, action_dim], dtype=tf.bool),
    )
    ...
    action_past_goal = action_chunk_indices > goal_timestep[:, None]  # [T, total_action_window]
