...
class TrainingStrategy(ABC):
    ...
    @abstractmethod
    def clip_grad_norm(self) -> Union[torch.Tensor, None]: ...
...
class JsonLogger:
    def __init__(self, run_dir: Path, run_id: str):
        ...
        self.f = open(run_dir / f"{run_id}.jsonl", "a", buffering=1)
    def log(self, obj: dict):
        self.f.write(json.dumps(obj, default=self._to_serializable) + "\n")
...
class EvalStrategy:
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        overwatch.info("Placing VLM on GPU for evaluation", ctx_level=1)
        ...
        overwatch.info(
            ...
            f"  |-> Global BSZ = {self.global_batch_size}\n"
            f"  |-> Stage = {self.stage}\n"
            f"  |-> VLM = {self.vlm}\n",
            ctx_level=1,
        )

        self.logger = JsonLogger(run_dir, "eval_run")
    ...
    # === VLA Latent Action Eval ===
    
    def run_latent_action_eval(
        self,
        vla_dataset: IterableDataset,
        collator: PaddedCollatorForActionPrediction,
        action_tokenizer: ActionTokenizer,
