...
class SelfAttention(nn.Module):
    ...
    def scaled_dot_product_attention(
        ...
        if attn_mask is not None:
            # attn_mask: (B, S) 기대
            attn_bias = attn_bias.unsqueeze(0).repeat(query.shape[0], 1, 1)  # (B, L, S)
            attn_bias.masked_fill_((attn_mask > 0).logical_not().unsqueeze(1), float("-inf"))
            attn_bias = attn_bias.unsqueeze(1)  # (B,1,L,S)

        attn_weight = query @ key.transpose(-2, -1) * self.scale  # (B,H,L,S)
        attn_weight += attn_bias
        attn_weight = torch.softmax(attn_weight, dim=-1)
        out = attn_weight @ value  # (B,H,L,D)
    ...
    def forward(
        ...
        out, attn_weight = self.scaled_dot_product_attention(q, k, v, is_causal=is_causal, attn_mask=attn_mask)
        out = rearrange(out, "b h n d -> b n (h d)")
        out = self.to_out(out)
        if return_attn:
            return out, attn_weight  # (B,N,D), (B,H,L,S)
        else:
            return out, None
...
class SpatioTemporalBlock(nn.Module):
    ...
    def forward(
        ...
        x_ = self.norm1(x)
        x_, attn_spatial = self.spatial_attn(x_, is_causal=False, attn_mask=attn_mask, return_attn=return_attn)
        x = x + x_
        ...
        x_ = self.norm2(x)
        if causal_temporal:
            x_, attn_temporal = self.temporal_attn(x_, is_causal=True, return_attn=return_attn)
        else:
            x_, attn_temporal = self.temporal_attn(x_, return_attn=return_attn)
        x = x + x_
        ...
        if return_attn:
            # attn_spatial:  (B*T, H, S, S)
            # attn_temporal: (B*S, H, T, T)
            return x, {"spatial": attn_spatial, "temporal": attn_temporal}
        else:
            return x, None
...
class SpatioTemporalTransformer(nn.Module):
    ...
    def forward(
        ...
        for block in self.transformer_blocks:
            x, attn_dict = block(x, self.causal_temporal, attn_mask, return_attn=return_attn)
            if return_attn:
...
class SpatioBlock(nn.Module):
    ...
    def forward(
        ...
        x = rearrange(x, "b t s e -> (b t) s e")
        x_ = self.norm1(x)
        x_, attn_spatial = self.spatial_attn(x_, attn_mask=attn_mask, return_attn=return_attn)
        x = x + x_
        x = rearrange(x, "(b t) s e -> b t s e", t=t_len)
...
class SpatioTransformer(nn.Module):
    ...
    def forward(
        ...
        attn_all = [] if return_attn else None
        for block in self.transformer_blocks:
            x, attn_dict = block(x, attn_mask=attn_mask, return_attn=return_attn)
            if return_attn:
                attn_all.append(attn_dict)
