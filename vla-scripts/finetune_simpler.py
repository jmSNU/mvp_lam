...
from prismatic.util.data_utils import PaddedCollatorForActionPrediction_Simpler
from prismatic.vla.action_tokenizer import ActionTokenizer
from prismatic.vla.datasets import RLDSBatchTransformSimpler, RLDSDataset
...
class FinetuneConfig:
    # fmt: off
    vla_path: str = "/path/to/vlm"           
    lam_path: str = "/path/to/mvp_lam"
    # Directory Paths
    data_root_dir: Path = Path("/path/to/dataset")      # Path to Open-X dataset directory
    dataset_name: str = "simpler"                   # Name of fine-tuning dataset (e.g., `droid_wipe`)
    ...
    learning_rate: float = 1e-3                                   # Fine-tuning learning rate
    grad_accumulation_steps: int = 8                                # Gradient accumulation steps
    image_aug: bool = False                                          # Whether to train with image augmentations
    shuffle_buffer_size: int = 8192                                # Dataloader shuffle buffer size (can reduce if OOM)
    ...
    # Tracking Parameters
    wandb_project: str = "fientune-Simpler"                          # Name of W&B project to log to (use default!)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    run_dir = (cfg.run_root_dir / exp_id).resolve()
    adapter_dir = (cfg.adapter_tmp_dir / exp_id).resolve()
    ...
    dec_params, lora_params, other_params = [], [], []
    for n, p in wrapped_model.named_parameters():
        if not p.requires_grad:
            continue
        if "action_decoder" in n:
            dec_params.append(p)
        elif "lora_" in n :
    ...
    param_groups = [
        {"params": dec_params, "lr": 1e-3, "weight_decay": 1e-3},
        {"params": lora_params, "lr": 2e-4, "weight_decay": 0.0},
    ]
    if len(other_params) > 0:
        param_groups.append({"params": other_params, "lr": 1e-4, "weight_decay": 1e-3})
    ...
    from latent_action_model.genie.modules.univla_lam import ControllableDINOLatentActionModel

    lam_cls = DINOLatentActionModel if "univla" not in cfg.lam_path else ControllableDINOLatentActionModel
    latent_action_model = lam_cls(
        in_dim=3,
    ...
    lam_ckpt = torch.load(cfg.lam_path)
    if "state_dict" in lam_ckpt:
        lam_ckpt = lam_ckpt["state_dict"]
    new_ckpt = {}
    ...
    batch_transform = RLDSBatchTransformSimpler(
        latent_action_model,
        processor.tokenizer,
    ...
    collator = PaddedCollatorForActionPrediction_Simpler(
            ...
            if gradient_step_idx > 0 and gradient_step_idx % cfg.save_steps == 0:
                if distributed_state.is_main_process:
                    ...
                    if not cfg.freeze_vla:
                        ...
                        wrapped_model.module.vla.save_pretrained(str(save_dir))
