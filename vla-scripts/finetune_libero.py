...
from transformers import AutoConfig, AutoImageProcessor, get_cosine_schedule_with_warmup
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if distributed_state.is_main_process:
        # date-based exp id prefix (Asia/Seoul local time on the machine)
    ...
    # save finetune config json (main process only)
    if distributed_state.is_main_process:
        cfg_path = run_dir / "finetune_config.json"
        with open(cfg_path, "w", encoding="utf-8") as f:
            json.dump(_jsonable(asdict(cfg)), f, ensure_ascii=False, indent=2, sort_keys=True)
    ...
    param_groups = [
        {"params": dec_params, "lr": 2e-4, "weight_decay": 1e-3},
        {"params": lora_params, "lr": 5e-5 , "weight_decay": 0.0},
    ]
    if len(other_params) > 0:
        param_groups.append({"params": other_params, "lr": 2e-5, "weight_decay": 1e-3})
    ...
    latent_action_model = DINOLatentActionModel(
        in_dim=3,
